    ...
    where F: Fn(&Vec<u8>) -> Vec<u8> + Clone
    {
        match self {
            ...
            Self::Identifier(id) => {
                ...
                // TODO - explicit conversion to Int
                Expr::Number(Number::Int(Integer::from_be_bytes([
...
mod tests {
    ...
    fn expr_numbers() {
        ...
        assert_eq!(
            init.eval( &|_|vec![0x0], &SEContext::default(&ArgTypes::new())), 
    ...
    fn expr_storage_read() {
        ...
        assert_eq!(
            init.eval( &|_| 5u32.to_be_bytes().to_vec(), &SEContext::default(&ArgTypes::new())), 
            Expr::Number(Number::Float(16.5))
        );
    }

    #[test]
    fn expr_storage_read_attr_accessor() {
        // 1.5 + (GET(_) * 3.0)
        let init = Expr::BinOp { 
            lhs: Box::new(Expr::Number(Number::Float(1.5))), 
            op: Op::Add, 
